...
@foreach($cartItems as $item)
    ...
    @php
        ...
        $productImage = $product ? (is_array($product) ? ($product['image'] ?? null) : ($product->image ?? null)) : null;
        $productName = $product ? (is_array($product) ? ($product['name'] ?? 'Product') : ($product->name ?? 'Product')) : 'Product';
        $productImage = $productImage ? $productImage : asset('frontend/assets/images/no-image.png');
